...
export class AdminResetPasswordUseCase implements IAdminResetPasswordUseCase {
    ...
    async execute(userId: string, password: string): Promise<AdminResetPasswordDto | null> {
        ...
        const updatedUser = await this._adminAuthRepository.update(
            userId,
            {
                passwordHash
            }
        ...
        if (!updatedUser) {
            throw new AppError(authConstants.RESET_PASSWORD_FAILED, HttpStatus.INTERNAL_SERVER_ERROR)
        }

        const updatedDomainUser = new Admin(updatedUser.id, updatedUser.name, updatedUser.email, updatedUser.passwordHash, updatedUser.createdAt, updatedUser.updatedAt)
